...
def test_case_config_error():
    ...
    match = re.compile(r".*ignore_case.*\n1:.*", re.MULTILINE)
    bad_syntax("a = select a where a==10", ignore_case=False, match=match)
    # without a hint of config issue
    match = re.compile(r"^\n1:.*", re.MULTILINE)
    bad_syntax("a = SELECT a WHERr a==10", ignore_case=False, match=match)


def test_assign_syntax():
    good_single_syntax("a = select a where a=10", ignore_case=True)
    good_single_syntax("a = select a where a==10", ignore_case=True)
...
def test_select_syntax():
    ...
    # no from
    good_syntax("select *", ignore_case=True)
    good_syntax("select * where a=100", ignore_case=True)
...
def test_load_save_syntax():
    ...
    good_single_syntax("save to 'x'", ["(header=True)"], ignore_case=True)
