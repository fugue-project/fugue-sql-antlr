...
fugueDropnaTask:
    DROP ROWS IF how=(ALL|ANY) (THENULL|THENULLS) (ON cols=fugueCols)? (FROM df=fugueDataFrame)?
    ;

fugueFillnaTask:
    FILL (THENULL|THENULLS) params=fugueParams (FROM df=fugueDataFrame)?
    ;
...
fugueTakeTask:
    TAKE (rows=INTEGER_VALUE (ROW|ROWS))? (FROM df=fugueDataFrame)? ((partition=fuguePrepartition)|(PRESORT presort=fugueColsSort))? ((THENULL|THENULLS) na_position=(FIRST|LAST))?
    ;
...
fugueOutputTask:
    OUTPUT (dfs=fugueDataFrames)? (partition=fuguePrepartition)? USING fugueUsing=fugueExtension (params=fugueParams)?
    ;
...
fugueOutputTransformTask:
    OUTTRANSFORM (dfs=fugueDataFrames)? (partition=fuguePrepartition)? USING fugueUsing=fugueExtension (params=fugueParams)? (CALLBACK callback=fugueExtension)?
    ;

fugueModuleTask:
    (assign=fugueAssignment)? SUB (dfs=fugueDataFrames)? USING fugueUsing=fugueExtension (params=fugueParams)?
    ;

fugueSqlEngine:
    CONNECT fugueUsing=fugueExtension (params=fugueParams)?
    ;
...
fugueSingleOutputExtensionCommonWild:
    USING fugueUsing=fugueExtension (params=fugueParams)? (SCHEMA schema=fugueWildSchema)?
    ;

fugueSingleOutputExtensionCommon:
    USING fugueUsing=fugueExtension (params=fugueParams)? (SCHEMA schema=fugueSchema)?
    ;
...
statement
    ...
    | CREATE theNamespace (IF NOT EXISTS)? multipartIdentifier
        (commentSpec |
         locationSpec |
         (WITH (DBPROPERTIES | PROPERTIES) tablePropertyList))*        #createNamespace
    | ALTER theNamespace multipartIdentifier
        SET (DBPROPERTIES | PROPERTIES) tablePropertyList              #setNamespaceProperties
    | ALTER theNamespace multipartIdentifier
        SET locationSpec                                               #setNamespaceLocation
    | DROP theNamespace (IF EXISTS)? multipartIdentifier
    ...
    | (DESC | DESCRIBE) theNamespace EXTENDED?
        multipartIdentifier                                            #describeNamespace
    | (DESC | DESCRIBE) TABLE? option=(EXTENDED | FORMATTED)?
        multipartIdentifier partitionSpec? describeColName?            #describeRelation
    | (DESC | DESCRIBE) QUERY? query                                   #describeQuery
    | COMMENT ON theNamespace multipartIdentifier IS
        comment=(STRING | THENULL)                                        #commentNamespace
    | COMMENT ON TABLE multipartIdentifier IS comment=(STRING | THENULL)  #commentTable
...
queryTerm
    ...
    | left=queryTerm
        theOperator=(INTERSECT | UNION | EXCEPT | SETMINUS) setQuantifier? right=queryTerm  #setOperation
    | left=queryTerm
        theOperator=INTERSECT setQuantifier? right=queryTerm                                #setOperation
    | left=queryTerm
        theOperator=(UNION | EXCEPT | SETMINUS) setQuantifier? right=queryTerm              #setOperation
    ;
...
sortItem
    : expression ordering=(ASC | DESC)? (THENULLS nullOrder=(LAST | FIRST))?
    ;
...
rowFormat
    ...
    | ROW FORMAT DELIMITED
      ...
      (THENULL DEFINED AS nullDefinedAs=STRING)?                                       #rowFormatDelimited
    ;
...
booleanExpression
    ...
    | left=booleanExpression theOperator=AND right=booleanExpression  #logicalBinary
    | left=booleanExpression theOperator=OR right=booleanExpression   #logicalBinary
    ;
...
valueExpression
    ...
    | theOperator=(MINUS | PLUS | TILDE) valueExpression                                        #arithmeticUnary
    | left=valueExpression theOperator=(ASTERISK | SLASH | PERCENT | DIV) right=valueExpression #arithmeticBinary
    | left=valueExpression theOperator=(PLUS | MINUS | CONCAT_PIPE) right=valueExpression       #arithmeticBinary
    | left=valueExpression theOperator=AMPERSAND right=valueExpression                          #arithmeticBinary
    | left=valueExpression theOperator=HAT right=valueExpression                                #arithmeticBinary
    | left=valueExpression theOperator=PIPE right=valueExpression                               #arithmeticBinary
...
primaryExpression
    ...
    | FIRST '(' expression (IGNORE THENULLS)? ')'                                                 #first
    | LAST '(' expression (IGNORE THENULLS)? ')'                                                  #last
...
qualifiedColTypeWithPosition
    : name=multipartIdentifier dataType (NOT THENULL)? commentSpec? colPosition?
    ;
...
colType
    : colName=errorCapturingIdentifier dataType (NOT THENULL)? commentSpec?
    ;
...
complexColType
    : identifier ':' dataType (NOT THENULL)? commentSpec?
    ;
...
alterColumnAction
    ...
    | commentSpec
    | colPosition
    | setOrDrop=(SET | DROP) NOT THENULL
    ;
